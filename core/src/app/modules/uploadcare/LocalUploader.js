...
const upload = (url, formdata, onProgress) => {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      xhr.open(
        "POST",
        `${process.env.REACT_APP_END_POINT_FILE_UPLOAD}?token=${token}`
      );
      xhr.onload = () => {
        try {
          const { status } = xhr;
...
export default class LocalUploader extends React.Component {
  static defaultProps = {
    ...
    onUploaded: () => {},
    fileType:
      "image/jpg, image/png, image/jpeg, application/msword, application/vnd.openxmlformats-officedocument.wordprocessingml.document, application/pdf",
    instruction: null
  };
  ...
  handleOnUpload = async e => {
    ...
    /*if(file_blob.type !== "application/pdf"){
            return Alert.warning("Invalid File Type");
        }*/
    ...
    formdata.append("file_url", args.file_blob);
    // formdata.append('name', args.name);
    // formdata.append('file_name', `${args.name}.pdf`);
              ...
              <div className="modal-content">
                ...
                <div className="modal-footer">
                  ...
                  {/*<button
                                        ...
                                        onClick={ this.handleOnUpload }>Proceed</button>*/}
                  <button
                    className="btn btn-outline-danger"
                    onClick={this.handleShowInstruction(false)}
                  >
